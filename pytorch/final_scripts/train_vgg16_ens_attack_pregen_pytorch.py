...
def vgg16_attack_pregen_training():
    ...
    patience = 3
    num_epochs = 100
    save_model_dir = "saved_model/vgg16_ens_attack_pregen_training/"
    ...
    for images, labels in train_loader:
        ...
        adv_images = projected_gradient_descent(resnet50_model, images, eps=0.1, eps_iter=0.01, nb_iter=10, norm=np.inf)
        train_adv_dataset.append([adv_images, labels])
        batch += 1
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in test_loader:
            ...
            # eval adv
        for images, labels in test_adv_dataset_list[0]:
        ...
        torch.save(model, f'{save_model_dir}/vgg16_ep_{epoch+1}_ens_attack_pregen_trained.pth')
        print(f"Saved {save_model_dir}/vgg16_ep_{epoch+1}_ens_attack_pregen_trained.pth")
    ...
    torch.save(model, f'{save_model_dir}/vgg16_attack_ens_pregen_best_model.pth')
    print(f"Saved {save_model_dir}/vgg16_attack_ens_pregen_best_model.pth")

def model_evaluation():
    premodel = torch.load(f'saved_model/vgg16_best_model.pth')
    model = torch.load(f'saved_model/vgg16_ens_attack_pregen_training/vgg16_attack_pregen_best_model.pth')
